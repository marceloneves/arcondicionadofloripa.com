(function(){
  ...
  if(form){const tel=form.querySelector('[name="telefone"]');if(tel){tel.addEventListener('input',()=>{tel.value=formatPhoneBR(tel.value);});}form.addEventListener('submit',async (e)=>{e.preventDefault();const nome=form.querySelector('[name="nome"]');const cidade=form.querySelector('[name="cidade"]');const servico=form.querySelector('[name="servico"]');const msg=form.querySelector('[name="mensagem"]');const notice=form.querySelector('.notice');const submitBtn=form.querySelector('button[type="submit"]');const errors=[];
  ...
  try{
    const response=await fetch('/api/contact',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nome:nome.value.trim(),telefone:tel.value.trim(),cidade:cidade.value,servico:servico.value,mensagem:msg.value.trim()})});
